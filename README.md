...
In this tutorial we will be building an online todo-list application. The focus of this tutorial is on the back-end side of website so I will not get deeper into the front-end side. We will learn how to use Rails with some great tools, such as:
...
- **List**:  Allow users to create a list which will contain different tasks.
-  **Task**: Allow users to create, delete and complete tasks.
- **User authentication and authorization**: Allow users to sign up and sign in. Restrict access only to signed in users. Authorize users before any action.
...
> **Note**: The source code is available on [Github repo](https://github.com/simposk/planner).
...
**Note**: if you don't have github set up on your computer go to [Github](https://github.com/) and create new account.  
Follow [this tutorial on how to set up ssh key.](http://devmarketer.io/learn/set-ssh-key-github/)

When you have everything set up and running, create a github repository:
...
##**Styling StaticPages**
We will be using bootstrap framework so we need to add `bootstrap` gem.
Inside `Gemfile` somewhere in the top add:

    gem 'bootstrap-sass', '~> 3.3.6'
In your terminal type

    $ bundle install
Following the installation [guide](https://github.com/twbs/bootstrap-sass), we need to add few lines:
Firstly, change  `app/assets/stylesheets/application.css` to  
`application.scss`.

Inside `application.scss` below comments add:
...
Inside `app/assets/javascripts/application.js` add:
	...
	...
We've successfully included bootstrap into application. Let's also include stylesheet reset so our webpage will look the same on all browsers. 
...
**Note**:`normalize.css` will be automatically included in
`application.scss` thanks to this line in `application.scss`:
...
It's usually a better idea to create partials so the code is clean and follows rails preferred **DRY** (Don't repeat yourselft) principle.
Create `layouts/_header.html.erb`  partial: 
...
We can see bootstrap has automatically added margin to the bottom of nav.
Let's remove it and add some styling:
...
##**Styling Lists** 
Open `app/views/lists/index.html.erb` and make it look like this:
...
Now go to `app/assets/stylesheets.application.scss` and import `font-awesome`:
	...
	@import "font-awesome";
	...
If we refresh our webpage, we should see "X" icons that delete lists if clicked. Remember we've done that by adding:
...
In `app/modes/task.rb`  rails has automatically declared `belongs_to :list` when we typed `list:references` in our terminal.
...
It tells us that we are missing partial `tasks/_form` so let's create it.
...
First, open terminal and generate migration:
    ...
    $ rails db:migrate
Now, open `tasks_controller.rb` and add complete action below `destroy`:
...
There are few things we still need to add:
...
**Note**: if get devise error, restart rails server(ctrl + c).
...
2. In action `create` we use
 `@list = current_user.lists.build(list_params)`
to create a new list with current user id. `current_user` is a devise helper method that was automatically generated.

If you refresh your website it should ask you to log in. Create an account and you will be redirected to `/lists` path

Go to `tasks_controller.rb` and also add: 
`before_action :authenticate_user!`
...
Let's create a partial for all alerts and notices inside 
